...
为了解决该问题，我们今天使用非阻塞的 NIO 来实现 Server 部分，使用设置为非阻塞的 `ServerSocketChannel` 绑定到监听的端口上，请求流由 `selector` 来调度到对应的 `channel` 处理，同时 `channel` 也负责响应。

最后的压测结果也显示对于一个简单的回声接口(`/echo`)基于 NIO 实现的 Server 进行 200线程100次请求压测结果为 100% 通过，而基于 BIO 实现的 Server 有近 20% 的请求被拒绝（失败）了。
